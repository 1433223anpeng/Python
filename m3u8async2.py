...
class WBDYM3U8():
    def __init__(self,txtindex,filename) -> None:
        ...
        # self.sessionA = AsyncHTMLSession()
    ...
    async def get_TS(self,url,num):
        while True:
            try:
                logger.info(f"第{num+1} 个片段正在请求 URL->{url}")
                async with self.sessionB.get(url) as resp:
                    resp.raise_for_status
                    con = await resp.content.read()
                    # r = await self.sessionA.get(url)
                    # print(r)
                    # r.raise_for_status
                    logger.info(f"第{num+1} 个片段请求成功 URL->{url}")
                    aes = AES.new(self.key,AES.MODE_CBC,self.key)
    ...
    async def notrootindex(self):
        self.sessionB = aiohttp.ClientSession()
        self.sessionB.headers.update(self.U_A)
        ...
        for i in range(0,len(url_all)):
            tasks.append(asyncio.ensure_future(self.get_TS(url_all[i],i)))
